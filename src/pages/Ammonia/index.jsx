...
function Ammonia() {
  const columns = [
    { key: "Date", value: "date" },
    { key: "PH Levels", value: "ph" },
    { key: "Dissolved oxygen", value: "dissolved_oxygen" },
    { key: "temperature", value: "temperature" },
    { key: "Predicted Ammonia", value: "predicted_ammonia" },
    { key: "Actual Ammonia", value: "actual_ammonia" },
  ];
 ...
 // get request to fetch all ponds of selected farm
  const fetchPonds = () => {
  ...
  useEffect(() => {
    ...
    instance
      .get(
        `ammonia/notes?farm=${farm}&pond=${pondId}&start_date=${startDate}&end_date=${endDate}`
      )
      .then((response) => {
    ...
    <>
      ...
      <div className="relative my-4  md:my-0 ">
        <select
          ...
          className="z-20 py-2 px-4 bg-white border border-[#D0D5DD] rounded-xl text-lg font-medium w-fit outline-none appearance-none"
        >
          <option className="text-primary text-[14px] " value="">
            All Ponds
          </option>
          {ponds.map((item) => {
            return (
              <option className="text-primary " key={item.id} value={item.id}>
                {item.name}
              </option>
            );
          })}
      ...
      {notes.length > 0 ? (
        <div className="text-[#20563F]  italic rounded-2xl gradient shadow-3xl  py-2 pe-4 ps-[38px] text-lg font-semibold mt-5 ">
          {notes.map((item) => {
            return (
              <p className="mb-3 md:mb-0" key={item}>
                {item}
              </p>
            );
          })}
