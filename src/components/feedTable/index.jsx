...
function FeedTable(props) {
  ...
  useEffect(() => {
    ...
    instance
      .get(
        `waterquality/feed/table?farm=${farm}&pond=${pondid}&start_date=${startDate}&end_date=${endDate}`
      )
      .then((response) => {
          ...
          <tbody className=" text-center ">
            {rows.slice(startIndex, endIndex + 1).map((row, index) => (
              <tr key={index}>
                ...
                {!row.editing && <td className="py-6 px-3">{row.feed_rate}</td>}
                {row.editing && (
                  <td className="py-6 ">
                    <input
                      type="text"
