...
function AmmoniaTable(props) {
  ...
  useEffect(() => {
    ...
    // GET request to fetch data of ammonia table
    instance
      .get(
        `waterquality/ammonia/table?farm=${farm}&pond=${pondid}&start_date=${startDate}&end_date=${endDate}  `
      )
      .then((response) => {
    ...
    <div className="bg-white rounded-[20px] shadow-3xl px-6 py-8 mt-8 ">
      ...
      <div className="relative my-4  md:my-0 lg:ms-3 ">
            <select
              ...
              className="z-20 py-2 px-2 bg-white border border-[#D0D5DD] rounded-xl text-base font-medium w-fit outline-none appearance-none"
            >
              <option value="null" >
                Sort By
              </option>
          ...
          <tbody className=" text-center">
            {sortedData.slice(startIndex, endIndex + 1).map((row, index) => (
              <tr key={index}>
                ...
                {!row.editing && <td className="py-6 px-3">{row.ph}</td>}
                {!row.editing && (
                  <td className="py-6 px-3">{row.dissolved_oxygen}</td>
                )}
                {!row.editing && (
